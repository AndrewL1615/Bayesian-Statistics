...
for(i in 1:length(k0)){
  ...
  s2.b <- (v0[i]*s20 + (n-1)*s20 + k0[i]*n*(ybara-mu0)^2/kn[i])/vn[i]
...
```
From the plot above, we can see that as $\kappa_{0} = \nu_{0}$ increases, the probability of $\theta_{A}<\theta_{B}|y_{A},y_{B}$ decreases. We can think of $\kappa_{0}, \nu_{0}$ as the prior sample size. Thus, if one has a very large sample size, they are less certain that $\theta_{A}<\theta_{B}|y_{A},y_{B}$ is true, where as someone with a smaller prior sample size would be more certain of $\theta_{A}<\theta_{B}|y_{A},y_{B}$
